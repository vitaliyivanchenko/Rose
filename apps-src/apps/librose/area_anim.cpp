...
static bool buf_use_texture = false;
...
void float_animation::redraw(texture& screen, const SDL_Rect& rect)
{
	...
	if (buf_use_texture) {
		if (buf_tex_.get()) {
			int width, height;
			SDL_QueryTexture(buf_tex_.get(), NULL, NULL, &width, &height);
			if (width != bufs_.first.w || height != bufs_.first.h) {
				buf_tex_ = NULL;
			}
		}
		if (!buf_tex_.get()) {
			buf_tex_ = SDL_CreateTexture(renderer, format.format, SDL_TEXTUREACCESS_TARGET, view_rect.w, view_rect.h);
		}
	} else {
		if (buf_surf_.get()) {
			int width = buf_surf_->w, height = buf_surf_->h;
			if (width < bufs_.first.w || height < bufs_.first.h) {
				buf_surf_ = NULL;
			}
		}
		if (!buf_surf_.get()) {
			buf_surf_ = create_neutral_surface(view_rect.w, view_rect.h);
		}
	}
	...
	if (buf_use_texture) {
		...
		texture_clip_rect_setter clip(NULL);
		{
			...
			SDL_RenderCopy(renderer, screen.get(), &view_rect, NULL);
		}
	} else {
		SDL_RenderReadPixels(renderer, &view_rect, buf_surf_->format->format, buf_surf_->pixels, 4 * view_rect.w);
	}
...
void float_animation::undraw(texture& screen)
{
	...
	if (buf_use_texture) {
		...
		{
			// trender_target_lock lock(renderer, screen);

			SDL_RenderCopy(renderer, buf_tex_.get(), NULL, &bufs_.first);
		}
	} else {
		SDL_UpdateTexture(screen.get(), &bufs_.first, buf_surf_->pixels, 4 * bufs_.first.w);
	}
}
...
void draw_canvas_anim(display& disp, int id, texture& canvas, const SDL_Rect& rect)
{
