...
void display::create_theme()
{
	...
	gui2::reload_window_builder(utils::generate_app_prefix_id("rose", game_config::theme_window_id), *theme_current_cfg_, reserve_wml_tag);
	...
	// below will add gui2::event::thandler to context. handler of controller_base must exist before it.
...
int display::screenshot(std::string filename, bool map_screenshot)
{
	int size = 0;
	if (!map_screenshot) {
		texture tex = screen_.getTexture();
		surface screenshot_surf = create_neutral_surface(screen_.getx(), screen_.gety());
		SDL_RenderReadPixels(get_renderer(), NULL, screen_.getformat().format, screenshot_surf->pixels, screen_.getx() * 4);
...
*/
	...
	BOOST_FOREACH (const tblit2 &blit3, drawing_buffer) {
		...
		BOOST_FOREACH (const image::tblit& blit, blits) {
			image::render_blit(renderer, screen, blit, blit3.x(), blit3.y());
		}
	}
...
*/
	...
	theme_->get_window()->draw_tooltip();
	...
	theme_->get_window()->undraw_tooltip();
}
		...
		Uint32 blendto, double blend_ratio, double submerged, bool vreverse)
{
	...
	if (submerged > 0.0) {
		...
		const int submerge_height = std::max<int>(0, image.height * (1.0 - submerged));
		// const int depth = surf->h - submerge_height;
		SDL_Rect srcrect = create_rect(0, 0, image.width, submerge_height);
		...
		if (submerge_height != image.height) {
			//the lower part will be transparent
			srcrect.y = submerge_height;
			srcrect.h = image.height - submerge_height;
			y += submerge_height;

			// drawing_buffer_add(drawing_layer, loc, x, y, surf, image.width, image.height, srcrect);
			image::tblit& material2 = drawing_buffer_add(drawing_layer, loc, x, y, *image.loc, image.loc_type, srcrect);
			material2.height = image::calculate_scaled_to_zoom(srcrect.h);
...
{
	return theme_->set_report_blits(num, blits);
}
...
bool display::scroll(int xmove, int ymove)
{
	...
	if (!screen_.update_locked()) {
		...
		texture target = SDL_CreateTexture(renderer, screen_.getformat().format, SDL_TEXTUREACCESS_TARGET, srcrect.w, srcrect.h);
		{
...
void display::draw(bool update,bool force) 
{
	...
	uint32_t ticks1, ticks2, ticks3, ticks4;
	...
	// posix_print("display::draw#%i, %u + %u(unrender) + %u(invalidated) + %u(commit) + %u\n", time ++, ticks1 - start_ticks, ticks2 - ticks1, ticks3 - ticks2, ticks4 - ticks3, stop_ticks - ticks4);
}
...
void display::clear_screen()
{
	...
	draw_rect2(get_renderer(), area, SDL_MapRGB(&get_screen_format(), 0, 0, 0));
}
...
void display::draw_image_for_report(surface& img, SDL_Rect& rect)
{
	...
	if (visible_area.x != 0 || visible_area.y != 0 || visible_area.w != img->w || visible_area.h != img->h) {
		...
		blit_from_surface(get_renderer(), img, &visible_area, &target);
	} else {
		...
		blit_from_surface(get_renderer(), img, NULL, &target);
	}
}
...
void display::draw_float_anim()
{
	...
	for (std::map<int, animation*>::iterator it = area_anims_.begin(); it != area_anims_.end(); ++ it) {
		...
		anim.redraw(video().getTexture(), rect);
	}
