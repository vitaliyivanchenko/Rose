...
treport::treport(bool multi_line, int unit_w, int unit_h, int gap)
...
void treport::replacement_children()
{
	...
	if (!multi_line_) {
		...
		content_grid_->replacement_children(unit_size_.x, unit_size_.y, gap_, multi_line_, fixed_cols_, *content_);
...
void treport::erase_children()
{
	if (!multi_line_) {
		...
		content_grid_->erase_children(unit_size_.x, unit_size_.y, gap_, multi_line_, fixed_cols_, *content_);
...
void treport::hide_children()
{
	if (!multi_line_) {
		...
		content_grid_->hide_children(unit_size_.x, unit_size_.y, gap_, multi_line_, fixed_cols_, *content_);
		// it must be same-size, to save time, don't calcuate linked_group.
		invalidate_layout(false);
	}
}
...
void treport::tabbar_init(bool toggle, const std::string& definition, bool always, int width, int height, int gap)
{
	...
	previous_->set_fix_size(unit_size_.x, unit_size_.y);
	...
	next_->set_fix_size(unit_size_.x, unit_size_.y);
...
void treport::tabbar_replacement_children()
{
	...
	if (!unit_size_.x) {
		...
		stuff_widget_->set_best_size2(spacer_width, grid_height);
