...
void ttoggle_panel::update_canvas()
{
	...
	BOOST_FOREACH(tcanvas& canva, canvas()) {
		...
		canva.set_variable("focussed_png", variant(focussed_png_));
		canva.set_variable("selected_png", variant(selected_png_));
	}
}
...
void ttoggle_panel::set_canvas_png(bool focussed, bool selected) 
{
	bool dirty = false;
	if (focussed_png_ != focussed) {
		focussed_png_ = focussed;
		dirty = true;
	}
	if (selected_png_ != selected) {
		selected_png_ = selected;
		dirty = true;
	}
	if (dirty) {
		update_canvas();
...
void ttoggle_panel::signal_handler_left_button_click(const event::tevent event, bool& handled, const int type)
{
	...
	if (did_state_pre_change_) {
		if (!did_state_pre_change_(*this, type)) {
			handled = true;
			return;
		}
	}
