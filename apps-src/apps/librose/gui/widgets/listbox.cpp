...
ttoggle_panel& tlistbox::add_row(const std::map<std::string /* widget id */, string_map>& data, const int index)
{
	...
	widget->set_did_state_pre_change(boost::bind(&tlistbox::did_pre_change, this, _1, _2));
	widget->set_did_state_changed(boost::bind(&tlistbox::did_changed, this, _1, _2));
...
void tlistbox::set_row_shown(const int row, const bool visible)
{
	...
	if (original_selected_row != get_selected_row() && cursel_ != npos && did_changed_) {
		did_changed_(*this, *list_grid_->child(cursel_, 0).widget_, drag_none);
	}
	invalidate_layout(false);
...
void tlistbox::select_row(const int row)
{
	twidget* desire_widget = NULL;
	if (row != twidget::npos) {
		const tgrid::tchild* children = list_grid_->children();
		int childs = list_grid_->children_vsize();
		if (row >= childs) {
		...
		desire_widget = children[row].widget_;

	}

	select_row2(desire_widget);
}

void tlistbox::select_row2(twidget* widget)
{
	ttoggle_panel* desire_panel = NULL;
	if (widget) {
...
void tlistbox::did_focus_changed(twidget& widget, const bool enter)
{
	if (did_focus_changed_) {
		did_focus_changed_(*this, widget, enter);
	}
}

bool tlistbox::did_pre_change(twidget& widget, const int type)
{
	if (did_pre_change_) {
		return did_pre_change_(*this, widget, type);
	}
	return true;
}

void tlistbox::did_changed(twidget& widget, const int type)
{
	/** @todo Hack to capture the keyboard focus. */
	get_window()->keyboard_capture(this);

	ttoggle_panel* clicked_panel = dynamic_cast<ttoggle_panel*>(&widget);
	...
	if (did_changed_) {
		did_changed_(*this, *(list_grid_->child(cursel_, 0).widget_), type);
	}
}

void tlistbox::did_click(twidget& widget, const int type)
{
	if (did_click_) {
		did_click_(*this, widget, type);
	}
}

void tlistbox::did_double_click(twidget& widget)
{
	if (did_double_click_) {
		did_double_click_(*this, widget);
	}
}
...
void tlistbox::list_impl_draw_children(texture& frame_buffer, int x_offset, int y_offset)
{
