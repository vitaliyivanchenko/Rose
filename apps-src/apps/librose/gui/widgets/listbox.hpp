...
public:
	...
	void select_row2(twidget* widget);
	...
	void set_drag_started(const boost::function<void (tlistbox& list, twidget& widget, tgrid& drag_grid, const int drag_at)>& callback)
		{ drag_started_ = callback; }

	void set_did_focus_changed(const boost::function<void(tlistbox&, twidget&, const bool)>& callback)
	{
		did_focus_changed_ = callback;
	}
	void set_did_pre_change(const boost::function<bool(tlistbox& list, twidget& widget, const int type)>& callback)
	{
		did_pre_change_ = callback;
	}
	void set_did_changed(const boost::function<void(tlistbox& list, twidget& widget, const int type)>& callback)
	{
		did_changed_ = callback;
	}
	void set_did_click(const boost::function<void(tlistbox& list, twidget& widget, const int type)>& callback)
	{
		did_click_ = callback;
	}
	void set_did_double_click(const boost::function<void(tlistbox& list, twidget& widget)>& callback)
	{
		did_double_click_ = callback;
	}
	void set_did_right_click(const boost::function<void(tlistbox& list, twidget& widget)>& callback)
	{
		did_right_click_ = callback;
	}
...
protected:
	...
	void did_focus_changed(twidget& widget, const bool);
	bool did_pre_change(twidget& widget, const int type);
	void did_changed(twidget& widget, const int type);
	void did_click(twidget& widget, const int type);
	void did_double_click(twidget& widget);
...
private:
	...
	void list_impl_draw_children(texture& frame_buffer, int x_offset, int y_offset);
	...
	boost::function<void (tlistbox& list, twidget& panel, tgrid& drag_grid, const int drag_at)> drag_started_;
	...
	boost::function<void(tlistbox& list, twidget& panel, const bool)> did_focus_changed_;
	boost::function<bool(tlistbox& list, twidget& panel, const int type)> did_pre_change_;
	boost::function<void(tlistbox& list, twidget& panel, const int type)> did_changed_;
	boost::function<void(tlistbox& list, twidget& panel, const int type)> did_click_;
	boost::function<void(tlistbox& list, twidget& panel)> did_double_click_;
	boost::function<void(tlistbox& list, twidget& panel)> did_right_click_;
