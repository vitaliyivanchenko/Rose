...
tscrollbar_container::tscrollbar_container(const unsigned canvas_count, bool listbox)
	...
	, need_layout_(false)
	, keep_content_grid_origin_(tpoint(npos, npos))
{
...
tpoint tscrollbar_container::calculate_best_size() const
{
	...
	const tpoint result2 = best_size_from_builder();
	unsigned w = result2.x != npos? result2.x: 0;
	unsigned h = result2.y != npos? result2.y: 0;

	const unsigned cfg_screen_width = settings::screen_width;
	const unsigned cfg_screen_height = settings::screen_height;
	if (reduce_width || w >= cfg_screen_width || h >= cfg_screen_height) {
...
void tscrollbar_container::place(const tpoint& origin, const tpoint& size)
{
	...
	if (keep_content_grid_origin_.x != npos) {
...
void tscrollbar_container::broadcast_frame_buffer(texture& frame_buffer)
{
...
void tscrollbar_container::clear_texture()
{
...
void tscrollbar_container::layout_children()
{
	if (need_layout_) {
		...
		tset_point_lock lock(keep_content_grid_origin_, npos);
