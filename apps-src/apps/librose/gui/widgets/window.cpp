...
bool twindow::set_orientation_resolution()
{
	...
	// update_screen_size();
		...
		posix_print("twindow::set_orientation_resolution, %ix%i\n", settings::screen_width, settings::screen_height);
			...
			*this, event::tdispatcher::front_child))
{
	...
	connect_signal<event::SDL_LEFT_BUTTON_DOWN>(
			  boost::bind(
				  &twindow::signal_handler_click_dismiss2, this, _2, _3, _4, _5)
	...
	connect_signal<event::MIDDLE_BUTTON_CLICK>(boost::bind(
				  &twindow::signal_handler_click_dismiss, this, _2, _3, _4)
			, event::tdispatcher::back_post_child);
	connect_signal<event::RIGHT_BUTTON_CLICK>(boost::bind(
				  &twindow::signal_handler_click_dismiss, this, _2, _3, _4)
			, event::tdispatcher::back_post_child);

	connect_signal<event::SDL_MOUSE_MOTION>(boost::bind(
				  &twindow::signal_handler_sdl_mouse_motion, this, _2, _3, _4, _5)
			, event::tdispatcher::front_child);
...
int twindow::show(const bool restore, const unsigned auto_close_timeout)
{
	...
	try {
		...
		if (restore) {
			...
			blit_from_surface(get_renderer(), restorer_, NULL, &rect);
...
void twindow::set_layer_draging(bool val, bool close) 
{ 
	layer_draging_ = val;
	if (!val) {
		layer_background_surf_ = NULL;
		layer_foreground_surf_ = NULL;

		if (close) {
			// since will close, forbit call draw() to avoid jitter.
			suspend_drawing_ = true;	
		}
	}
	set_dirty();
...
void twindow::draw()
{
	...
	if (layer_draging_ && layer_background_surf_) {
		owner_->draw_layer(frame_buffer, layer_background_surf_, layer_foreground_surf_);
		dirty_list_.clear();
		return;
	}
	...
	bool first_draw_background = true;
	if (layer_draging_) {
		// will enter layer-drag mode. first splilt background and foreground.
		VALIDATE(!layer_foreground_surf_ && !layer_background_surf_, null_str);

		layer_foreground_surf_ = SDL_CreateTexture(get_renderer(), video_.getformat().format, SDL_TEXTUREACCESS_TARGET, settings::screen_width, settings::screen_height);
		SDL_SetTextureBlendMode(layer_foreground_surf_.get(), SDL_BLENDMODE_BLEND);

		layer_background_surf_ = SDL_CreateTexture(get_renderer(), video_.getformat().format, SDL_TEXTUREACCESS_TARGET, settings::screen_width, settings::screen_height);

		// target to layer_foreground_surf_ texture.
		frame_buffer = layer_background_surf_;
		SDL_SetRenderTarget(get_renderer(), frame_buffer.get());
...
*/
	...
	BOOST_FOREACH(std::vector<twidget*>& item, dirty_list_) {
		...
		if (!is_theme()) {
			// Restore.
			blit_from_surface(get_renderer(), restorer_, NULL, &window_rect);
		}
		...
		for (std::vector<twidget*>::iterator itor = item.begin(); itor != item.end(); ++ itor) {
			...
			if (first_draw_background) {
				first_draw_background = false;
				if (layer_draging_) {
					frame_buffer = layer_foreground_surf_;
					SDL_SetRenderTarget(get_renderer(), frame_buffer.get());
					SDL_RenderClear(get_renderer());
				}
			}
...
*/
	if (layer_draging_) {
		...
		// recover to default texture.
		frame_buffer = video_.getTexture();
		SDL_SetRenderTarget(get_renderer(), frame_buffer.get());

		// target of this draw is split background and foreground. app may modify foreground different with normal.
		// in order to no jitter, keep original scren textrue.
/*
		SDL_RenderCopy(get_renderer(), layer_background_surf_.get(), NULL, NULL);
		SDL_RenderCopy(get_renderer(), layer_foreground_surf_.get(), NULL, NULL);
*/
	}
...
void twindow::layout()
{
	...
	VALIDATE(!fix_coordinate_ || (maximum_width == settings::screen_width && maximum_height == settings::screen_height), null_str);
...
{
	VALIDATE(fix_coordinate_, null_str);
...
tpoint twindow::calculate_best_size() const
{
	if (!fix_coordinate_) {
		...
		return tpoint(settings::screen_width, settings::screen_height);
	}
}
...
void twindow::insert_tooltip(const std::string& msg, const twidget& widget)
{
	...
	tooltip_surf_ = SDL_CreateTexture(renderer, video_.getformat().format, SDL_TEXTUREACCESS_TARGET, w, h);
	tooltip_buf_.assign(create_neutral_surface(w, h));

	tcanvas& _canvas = canvas(1);
	_canvas.set_variable("width", variant(w / twidget::hdpi_scale));
	_canvas.set_variable("height", variant(h / twidget::hdpi_scale));
	_canvas.set_variable("dwidth", variant(w));
	_canvas.set_variable("dheight", variant(h));
	_canvas.set_variable("tip_text_maximum_width", variant(maximum_width / twidget::hdpi_scale));
	...
	trender_target_lock lock(renderer, tooltip_surf_);
	bool blend_none = true;
	for (std::vector<tcanvas::tshape_ptr>::iterator it = tip.begin(); it != tip.end(); ++ it) {
		(*it)->draw(tooltip_surf_, w, h, _canvas.variables(), blend_none);
		blend_none = false;
	}
}

// draw tooltip to screen
void twindow::draw_tooltip()
{
	if (tooltip_surf_) {
		...
		{
			surface_lock dst_lock(tooltip_buf_);
			SDL_RenderReadPixels(renderer, &rect, video().getformat().format, tooltip_buf_->pixels, 4 * rect.w);
		}

		SDL_RenderCopy(renderer, tooltip_surf_.get(), NULL, &rect);
		// sdl_blit(screen, &rect, tooltip_buf_, NULL);
		// sdl_blit(tooltip_surf_, NULL, screen, &rect);
	}
}

void twindow::undraw_tooltip()
{
	if (tooltip_surf_) {
		SDL_Rect rect = tooltip_rect_;
		blit_from_surface(get_renderer(), tooltip_buf_, NULL, &rect);
...
{
	posix_print("signal_handler_sdl_video_resize(%s), new_size: %ix%i\n", id().c_str(), new_size.x, new_size.y);
...
void twindow::signal_handler_click_dismiss2(
...
void twindow::signal_handler_sdl_mouse_motion(
		  const event::tevent event, bool& handled, bool& halt, const tpoint& coordinate)
{
