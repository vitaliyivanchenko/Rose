...
// #define ctrlid_tpl_chat		"_tpl_chat_"
...
std::vector<tchat_::tcookie>& tchat_::ready_branch(bool person, const tlobby_channel& channel)
{
	...
	if (!person) {
		toggle.set_did_state_changed(boost::bind(&tchat_::contact_channel_toggled, this, _1));
	}
...
void tchat_::insert_user(bool person, std::vector<tcookie>& branch, const tlobby_user& user)
{
	if (person) {
		...
		toggle.set_data(signature_ ++);
		if (person) {
			toggle.set_did_state_changed(boost::bind(&tchat_::contact_person_toggled, this, _1));
...
*/
	...
	notify_list_->set_did_changed(boost::bind(&tchat_::notify_toggled, this, boost::ref(window), _1, _3));
	notify_list_->set_did_click(boost::bind(&tchat_::notify_did_click, this, _1, _2, _3));
#ifdef _WIN32
	notify_list_->set_did_double_click(boost::bind(&tchat_::notify_did_double_click, this, _1, _2));
#endif
	chanlist_->set_did_changed(boost::bind(&tchat_::find_chan_toggled, this, boost::ref(window), _1, _3));
	...
	if (portrait_) {
		...
		catalog_->multiline_init(true, "image_tab");
	}
	...
	switch_to_home(window);
	// contact_switch_to_person(window);
...
void tchat_::contact_switch_to_person(twindow& window)
{
	...
	if (person_tree_->selected_item()) {
		ttoggle_panel& toggle = *(person_tree_->selected_item()->label());
		contact_person_toggled(toggle);
	}
}
...
void tchat_::did_timer_vrenderer(ttrack& widget, const tpoint& offset, const bool from_timer, bool force)
{
	...
	texture_clip_rect_setter lock(&widget_rect);
...
bool tchat_::did_control_drag_detect(tcontrol* control, bool start, const twidget::tdrag_direction type)
{
	if (!start) {
		original_local_offset_ += current_local_offset_;
		current_local_offset_.x = current_local_offset_.y = 0;

		int width = control->get_width();
		int height = control->get_height();
...
bool tchat_::did_drag_coordinate(tcontrol* control, const tpoint& first, const tpoint& last)
{
	current_local_offset_ = last - first;
	did_timer_vrenderer(*vrenderer_track_, vrenderer_track_->get_frame_offset(), true, true);
...
void tchat_::switch_to_video(twindow& window)
{
	...
	if (!vrenderer_track_) {
		...
		track->set_did_timer(boost::bind(&tchat_::did_timer_vrenderer, this, _1, _2, _3, false));
		track->set_did_control_drag_detect(boost::bind(&tchat_::did_control_drag_detect, this, _1, _2, _3));
		track->set_did_drag_coordinate(boost::bind(&tchat_::did_drag_coordinate, this, _1, _2, _3));
...
void tchat_::notify_toggled(twindow& window, tlistbox& list, const int type)
{
...
cricket::VideoCapturer* tchat_::OpenVideoCaptureDevice() 
{
...
void tchat_::refresh_vrenderer_status(twindow& window) const
{
	...
	label.set_label(tintegrate::generate_format(ss.str(), "white"));
}
		...
		if (state_ == SIGNING_IN) {
			...
			if (response == "accepted") {
				...
				if (setup_caller) {
					...
					tlabel& label = find_widget<tlabel>(vrenderer_layer_, ctrlid_vrenderer_status, false);
					// label.set_label(tintegrate::generate_format(_("Is waiting for the other to answer"), "white"));
					label.set_label(tintegrate::generate_format(_("Calling"), "white"));
				}
