...
{
	lower = utils::transform_tolower(name);
}
...
void tbrowse::pre_show(CVideo& /*video*/, twindow& window)
{
	...
	list.set_did_changed(boost::bind(&tbrowse::item_selected, this, boost::ref(window), _1, _3));
...
void tbrowse::update_file_lists(twindow& window)
{
...
#ifdef _WIN32
		files_in_current_dir_.insert(tfile2(conv_ansi_utf8_2(str, true)));
#else
	...
	for (std::vector<std::string>::const_iterator it = dirs.begin(); it != dirs.end(); ++ it) {
		const std::string& str = *it;
#ifdef _WIN32
		dirs_in_current_dir_.insert(tfile2(conv_ansi_utf8_2(str, true)));
#else
