...
{
	VALIDATE(std::find(dispatchers_.begin(), dispatchers_.end(), dispatcher) == dispatchers_.end(), null_str);
...
void thandler::disconnect(tdispatcher* dispatcher)
{
	...
	std::vector<tdispatcher*>::iterator itor = std::find(dispatchers_.begin(), dispatchers_.end(), dispatcher);
...
void thandler::mouse(const tevent event, const tpoint& position)
{
	if (mouse_focus) {
		...
		for (std::vector<tdispatcher*>::reverse_iterator ritor = dispatchers_.rbegin(); ritor != dispatchers_.rend(); ++ritor) {
			tdispatcher& dispatcher = **ritor;
			if (dispatcher.get_mouse_behaviour() == tdispatcher::all) {
				dispatcher.fire(event, *dynamic_cast<twidget*>(*ritor), position);
				break;
			}
			if (dispatcher.get_mouse_behaviour() == tdispatcher::none) {
				continue;
			}
...
bool is_in_dialog()
{
	...
	if (event::handler->dispatchers_.size() == 1) {
		event::tdispatcher* dispatcher = event::handler->dispatchers_.front();
		if (dispatcher->is_theme()) {
			return false;
		}
	}
	return true;
