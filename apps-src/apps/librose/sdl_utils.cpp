...
void draw_line2(SDL_Renderer* renderer, Uint32 argb, int x1, int y1, int x2, int y2)
{
	SDL_Color color = int_to_color(argb);
	trender_draw_color_lock lock(renderer, color.r, color.g, color.b, color.a);
	SDL_RenderDrawLine(renderer, x1, y1, x2, y2);
}

void draw_rect2(SDL_Renderer* renderer, const SDL_Rect& rect, Uint32 argb)
{
	SDL_Color color = int_to_color(argb);
	trender_draw_color_lock lock(renderer, color.r, color.g, color.b, color.a);
...
surface create_optimized_surface(const surface &surf)
{
	...
	const SDL_PixelFormat* format = surf->format;
	if (!is_display_surface(surf)) {
		surface const result = make_display_surface(surf);
		if (result == NULL) {
			return surf;
		}
...
void blit_from_surface(SDL_Renderer* renderer, const surface& surf, const SDL_Rect* srcrect, const SDL_Rect* dstrect)
{
	if (surf) {
