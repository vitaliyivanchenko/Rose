...
{
    posix_print("tstep::execute, task: %s, current_step_: %i\n", ble.current_task_->id().c_str(), ble.current_step_);
...
{
	VALIDATE(at == gui2::twidget::npos, null_str);
...
{
	if (start) {
        posix_print("background_reconnect_detect, start\n");
...
void tble::start_scan()
{
    ...
    posix_print("------tble::start_scan------uuid: %s\n", uuid? uuid: "<nil>");
	SDL_BleScanPeripherals(uuid);
}

void tble::stop_scan()
{
    posix_print("------tble::stop_scan------\n");
...
void tble::did_release_peripheral(SDL_BlePeripheral& peripheral)
{
	...
	posix_print("tble::did_release_peripheral, name: %s\n", peripheral.name);
...
{
	posix_print("tble::did_connect_peripheral, name: %s, connecting_peripheral_: %p, error: %i, persist_scan_: %s\n", peripheral.name, connecting_peripheral_, error, persist_scan_? "true": "false");
...
{
	posix_print("tble::did_disconnect_peripheral, name: %s\n", peripheral.name);
	...
	if (!disable_reconnect_) {
		posix_print("tble::did_disconnect_peripheral, start_scan\n");
		start_scan();
	}
}
...
{
	posix_print("tble::did_notify_characteristic(%s), current_task: %s, current_step_: %i\n", characteristic.uuid, current_task_? current_task_->id().c_str(): "null", current_step_);
...
bool tble::make_characteristic_exist(const std::string& service_uuid, const std::string& characteristic_uuid)
{
	...
	if (!peripheral->valid_services) {
		...
		if (!service) {
			posix_print("make_characteristic_exist, %s\n", err.str().c_str());
		}
...
{
    posix_print("tble::task_next_step, current_task: %s, finished_step_: %i\n", current_task_->id().c_str(), current_step_);
...
std::string mac_addr_uc6_2_str(const unsigned char* uc6)
{
	...
	return utils::transform_toupper(ss.str());
}
