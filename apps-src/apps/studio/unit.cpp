...
void unit::redraw_widget(int xsrc, int ysrc, int width, int height) const
{
	...
	if (!fix_rect()) {
		...
		if (surf) {
			disp_.drawing_buffer_add(display::LAYER_UNIT_DEFAULT, loc_, xsrc + width - surf->w, ysrc, surf, 0, 0);
		}
	}
...
config unit::generate2_change2_cfg(const gui2::tcell_setting& cell) const
{
	...
	config cfg;
	cfg["definition"] = utils::generate_app_prefix_id(widget_.second->app, widget_.second->id);
	if (is_report()) {
