...
void tcontrol_setting::set_definition(twindow& window)
{
	...
	const std::string current_definition = advanced_current_tab_? current_advanced_cfg_["definition"]: utils::generate_app_prefix_id(u_.widget().second->app, u_.widget().second->id);
...
void tcontrol_setting::set_advanced_misc(twindow& window, const config& cfg, bool enable)
{
	...
	set_definition_label(window, cfg["definition"].str());
