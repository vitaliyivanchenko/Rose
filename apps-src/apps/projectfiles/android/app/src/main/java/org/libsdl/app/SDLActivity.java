    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        if (requestCode == REQUEST_CAMERA_PERMISSION) {
            if (grantResults[0] == PackageManager.PERMISSION_DENIED) {
                // close the app
                Log.e(TAG, "Sorry!!!, you can't use this app without granting permission");
            }
        }
    }

    public static boolean checkWebrtcPermission(Context context, String[] MANDATORY_PERMISSIONS)
    {
        for (String permission : MANDATORY_PERMISSIONS) {
            if (ActivityCompat.checkSelfPermission(context, permission) != PackageManager.PERMISSION_GRANTED) {
                Log.e(TAG, "Permission " + permission + " is not granted, request...");
                ActivityCompat.requestPermissions(mSingleton, new String[]{permission}, REQUEST_CAMERA_PERMISSION);
/*
                if (ActivityCompat.checkSelfPermission(context, permission) != PackageManager.PERMISSION_GRANTED) {
                    Log.e(TAG, "User doesn't grant permission " + permission);
                    return false;
                }
*/
                return false;
